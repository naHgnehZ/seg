...
class Segmentation():
    def __init__(self, cfg, model=None):
        ...
        if model is not None:
            ...
            self.model = MaskFormerModel(cfg, device='cuda')
    ...
    def forward(self, img_list=None):        
        ...
        for image_path in tqdm.tqdm(img_list):
            ...
            negatives = torch.zeros((input_tensor.shape[0], input_tensor.shape[-1], input_tensor.shape[-1])).to(device=self.device).long()
            text_input_ids = torch.zeros((input_tensor.shape[0], self.cfg.DATASETS.TEXT_MAX_LEN)).to(device=self.device).long()
            text_attention_mask = torch.zeros((input_tensor.shape[0], self.cfg.DATASETS.TEXT_MAX_LEN)).to(device=self.device).long()
            text_token_type_ids = torch.zeros((input_tensor.shape[0], self.cfg.DATASETS.TEXT_MAX_LEN)).to(device=self.device).long()

            outputs = self.model(input_tensor, negatives, text_input_ids, text_attention_mask, text_token_type_ids)
            ...
            mask_img = np.round(mask_img)
            mask_img[mask_img == 1] = 125
            mask_img[mask_img == 2] = 255
            mask_images.append(mask_img)
            
            img = Image.fromarray(mask_img.astype(np.uint8))
            img.save(os.path.join(self.output_dir, os.path.basename(image_path)))
        return mask_images
